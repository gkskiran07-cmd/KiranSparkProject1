...
os.environ['JAVA_HOME'] = r'C:\Users\Kirankumar\.jdks\corretto-1.8.0_492'        #  <----- 🔴JAVA PATH🔴
######################🔴🔴🔴################################
...
df = spark.createDataFrame(data, ["name", "city", "amt"])

df.createOrReplaceTempView("df")

df.show()

agg_func = ( df.groupBy("name").agg(
    sum("amt").alias("Total"),
    count("amt").alias("Count"),
    collect_list("amt").alias("Collect"),
    collect_set("amt").alias("amt_distinct")

))
agg_func.show()

spark.sql("""  select name, sum(amt) as total,
                      count(amt) as cnt,
                      collect_list(amt) as amt_collect,
                      collect_set(amt) as amt_distinct from df group by name,city
          """).show()
